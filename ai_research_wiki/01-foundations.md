...
BatchNorm 的 batch 统计噪声可能产生附带正则化效果，但它的首要作用是改善优化与数值尺度，不应把它当作通用防过拟合方法。小 batch、序列模型或分布变化时，这种效果也不稳定。
...
- 小 batch：梯度噪声大，可能泛化好，但训练不稳定。
- 大 batch：吞吐高，梯度稳，但显存大，可能需要 learning-rate warmup 和调参。
